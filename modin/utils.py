...
_doc_inheritance_kwargs_by_object = dict()
...
def _update_inherited_docstrings(doc_module):
    for cls_or_func, kwargs in _doc_inheritance_kwargs_by_object.items():
        _inherit_docstrings_in_place(cls_or_func, doc_module.get(), **kwargs)
...
def _inherit_docstrings(
    ...
    def decorator(cls_or_func: Fn) -> Fn:
        _inherit_docstrings_in_place(cls_or_func, DocModule.get(), parent, excluded, overwrite_existing, apilink)
        _doc_inheritance_kwargs_by_object[cls_or_func] = {"parent": parent, "excluded": excluded, "overwrite_existing": overwrite_existing, "apilink": apilink}
        return cls_or_func
